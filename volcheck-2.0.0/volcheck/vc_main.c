...
static Bool clo_missing_loc_ok    = False;
...
static Bool vc_process_cmd_line_option(const HChar* arg)
{
   ...
   else if VG_BOOL_CLO(arg, "--print-mem-per",     clo_print_mem_per) {}
   else if VG_BOOL_CLO(arg, "--print-mem-summary", clo_print_mem_summary) {}
   else if VG_BOOL_CLO(arg, "--missing-loc-ok",    clo_missing_loc_ok) {}
   else
      return False;
...
static const HChar* nameOfTypeIndex ( IRType ty )
{
   switch (ty) {
...
lookup_symbol(Addr addr)
{
   SegInfo* seg = VG_(find_seginfo)(addr);
   Int      how_many;
   Int      i;
   ...
   how_many = VG_(seginfo_syms_howmany)(seg);
   for (i = 0; i < how_many; ++i) {
      ...
      VG_(seginfo_syms_getidx)(seg, i,
                   &sym_addr, &sym_tocptr, &sym_size, &sym_name);
      if (sym_addr == addr) {
          return sym_name;
      }
   }
...
static AddrRecord addr_table[ADDR_TABLE_SIZE];
static int addr_table_cnt = 0;
static AddrRecord addr_table_root; /* Root of the linked list in table */

static AddrRecord addr_record_scratch;

static void
init_addr_record(AddrRecord *r)
{
   int i;

   r->addr = 0;
   r->symbol = 0;
   for (i = 0; i < 4; ++i) { r->loads[i] = 0; }
   for (i = 0; i < 4; ++i) { r->stores[i] = 0; }
   for (i = 0; i < 4; ++i) { r->modifies[i] = 0; }
}

static AddrRecord *
alloc_addr_record(void)
{
   AddrRecord *r;

   if (addr_table_cnt >= ADDR_TABLE_SIZE) {
      /*
       * Assert now, to make allocation errors immediately apparent.
       *
       * But: the callers of this function should be prepared to deal with
       * allocation errors at run time.  Remove the assertion if you prefer
       * that behavior.
       */
      tl_assert(!"address table is full");
      return 0;
   }

   r = &(addr_table[addr_table_cnt]);
   ++addr_table_cnt;
   init_addr_record(r);
...
static void
init_addr_table(void)
{
   init_addr_record(&addr_table_root);
   addr_table_cnt = 0;
}

static AddrRecord *
find_addr_record(Addr addr)
{
   AddrRecord *p = &addr_table_root;
   AddrRecord *next;
   AddrRecord *new;

   /*
    * Find the record for `addr', or find the place to insert a new record for
    * `addr' into the list.
    */
   while (1) {
      next = p->next;
   ...
   /*
    * If we get here, then `p' points to the record that should precede the
    * newly allocate record for `addr'.
    */
   new = alloc_addr_record();
   if (!new) {
      return 0;
   }
   new->addr = addr;
   new->symbol = lookup_symbol(addr);
   new->next = next;
   p->next = new;
   return new;
}

static const AddrRecord *
record_event(Addr addr, SizeT size, EventKind kind)
{
   AddrRecord *r = find_addr_record(addr);
   ULong *counts;
   int i;
   
   if (!r) {
      // XXX print out a warning
      init_addr_record(&addr_record_scratch);
      addr_record_scratch.addr = addr;
      addr_record_scratch.symbol = lookup_symbol(addr);

      return &addr_record_scratch;
   }
   
   switch (kind) {
   case Event_Dr:    counts = r->loads;    break;
   case Event_Dw:    counts = r->stores;    break;
   case Event_Dm:    counts = r->modifies;    break;
   default:        tl_assert(0);        break;
   }
   switch (size) {
   case 1:    i = 0;        break;
...
{
   switch (kind) {
   case Event_Ir: return "ifetch"; break;
   case Event_Dr: return "load"; break;
   case Event_Dw: return "store"; break;
   case Event_Dm: return "modify"; break;
   }
   return "unknown";
}

static void
trace_event(Addr addr, SizeT size, EventKind kind)
{
   const AddrRecord *record;
   
   if (!clo_missing_loc_ok) {
      /* Only trace activities within the program source text. */
      if (!loc_available)
     return;
   }
   /* Only trace globals. */
   ...
   if (clo_print_mem_per) {
      HChar *symbol_name = record->symbol;
      if (!symbol_name) {
     symbol_name = "<unknown>";
      }
      if (loc_available) {
     VG_(printf)("  %s:%u", loc_filename, loc_linenum);
      } else {
     VG_(printf)("  ???:???");
      }
      VG_(printf)(":  %s %s    (%08p, %d)\n",
          event_name(kind), symbol_name, addr, size);
   }
}

static VG_REGPARM(2) void
trace_instr(Addr addr, SizeT size)
{
/*
...
{
  trace_event(addr, size, Event_Dr);
}

static VG_REGPARM(2) void 
trace_store(Addr addr, SizeT size)
{
  trace_event(addr, size, Event_Dw);
}

static VG_REGPARM(2) void 
trace_modify(Addr addr, SizeT size)
{
  trace_event(addr, size, Event_Dm);
}
...
static void vc_fini(Int exitcode)
{
   ...
   VG_(umsg)("Volcheck recorded access info for %d unique memory accesses\n",
              addr_table_cnt);
   if (addr_table_cnt >= ADDR_TABLE_SIZE) {
      VG_(umsg)("WARNING: Volcheck may have missed some accesses due to table exhaustion\n");
   }
   if (clo_print_mem_summary) {
      AddrRecord *p;
      VG_(umsg)("Below is a memory-access summary (op, addr, size, count)\n");
      VG_(umsg)("\n");
      for (p = addr_table_root.next; p != 0; p = p->next) {
         print_addr_accesses(p, p->loads, event_name(Event_Dr));
         print_addr_accesses(p, p->stores, event_name(Event_Dw));
         print_addr_accesses(p, p->modifies, event_name(Event_Dm));
#if 0
         VG_(printf)("  %08p", p->addr);
         if (p->symbol)
            VG_(printf)(" %s\n", p->symbol);
         else
            VG_(printf)(" <unknown>\n");
